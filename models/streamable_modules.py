...
class Module(ABC):
    """This abstract base class defines functions to make torch.nn Modules streamable.
    Any inheriting subclass has to have the same name as its stationary equivalent, e.g. Linear, Conv1d or ReflectionPad1d.
    Furthermore it has to implement the functions:
    - __init__ with a call to super.
    - __forward_and_initialize_state__
    - __forward_and_propagate_state__
    - __forward_and_finish_state__
    ...
    def final_slice() -> None:
        ...
        Outputs:
        - None"""
        Module.__is_final_slice__ = True
    ...
    def close_stream() -> None:
        ...
        Outputs:
        - None"""
        Module.__is_final_slice__ = False
    ...
    def __ensure_data_precision__(x:  Union[torch.Tensor, List[torch.Tensor]]) -> torch.Tensor:
        ...
        if type(x) == type([]) or type(x) == type(()):
            for i in range(len(x)):
                if not x[i].dtype == Module.precision:
                    x[i] = x[i].to(Module.precision)
        else: 
            if not x.dtype == Module.precision:
                 x = x.to(Module.precision)
    ...
    def __call__(self, input: Union[torch.Tensor, List[torch.Tensor]]) -> torch.Tensor:
        ...
        input = Module.__ensure_data_precision__(x=input)
        
        # Initial call
        if self.__state__ == None: 
            y_hat = self.__forward_and_initialize_state__(x=input)
        # Final call
        elif Module.__is_final_slice__: 
...
class Conv1d(Module, torch.nn.Conv1d):
...
class ConvTranspose1d(Module, torch.nn.ConvTranspose1d):
    ...
    def __init__(self, **kwargs) -> object:
        ...
        Inputs:
        - **kwargs: the same arguments as for a regular torch.nn.Conv1d. Note that output_padding is not supported.
        ...
        if "output_padding" in kwargs.keys() and kwargs["output_padding"] != 0:
            raise Exception("Any non-zero value for output padding is not supported.")
